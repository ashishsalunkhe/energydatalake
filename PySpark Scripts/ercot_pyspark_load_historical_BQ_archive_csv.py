...
spark = SparkSession.builder \
    .appName("ercotLoadHistoricalApp") \
    .config("spark.jars.packages", "com.google.cloud.spark:spark-bigquery-with-dependencies_2.12:0.22.0") \
    .getOrCreate()

# Cloud storage paths
folder_path = "gs://ercot_test/ercot_load_csv/load_historical"

destination_table = "driven-stage-365620.ercot_merged.ercot_load_historical"
...
if not check_folder_has_files("ercot_test", "ercot_load_csv/load_historical"):
    ...
    df_load_historical = spark.read.option("header", "true").csv(folder_path)
# Reading and cleaning dataframes

#Column name changes
df_load_historical_clean = df_load_historical.select([col(c).alias(c.replace(' ', '_').lower()) for c in df_load_historical.columns])

#Logging-1
logging.info(f"Number of rows in the dataframe before datatype transformation: {df_load_historical_clean.count()}")

#Printing the combined df before transformation
print("Schema of the dataframe:")
df_load_historical_clean.printSchema()

df_load_historical_clean.show(5)

null_counts = df_load_historical_clean.select([sum(col(c).isNull().cast('int')).alias(c) for c in df_load_historical_clean.columns])
#Logging-2
...
df_load_historical_clean = df_load_historical_clean.withColumn('time', to_timestamp(col('time')))
df_load_historical_clean = df_load_historical_clean.withColumn('interval_start', to_timestamp(col('interval_start')))
df_load_historical_clean = df_load_historical_clean.withColumn('interval_end', to_timestamp(col('interval_end')))

# Converting other columns to float and handling null values
float_columns = ['load']
decimal_type = DecimalType(10, 2)
for col_name in float_columns:
     df_load_historical_clean = df_load_historical_clean.withColumn(col_name, col(col_name).cast(decimal_type))

#Dropping Null values
df_load_historical_clean = df_load_historical_clean.na.drop()
# Drop duplicate rows
df_load_historical_clean = df_load_historical_clean.dropDuplicates()
...
df_load_historical_clean.write.format('bigquery') \
...
archive_configurations = [
    
    {
        ...
        "source_folder_name": "ercot_load_csv/load_historical",
